...
// Dynamic form templates based on test type
...
export default function EditPersonalityTest() {
  ...
  const loadTest = async () => {
    ...
    setIsLoading(true);
    try {
      const test = await PersonalityTest.get(id);
      
      // Pre-fill form data
      setFormData({
        test_type: test.test_type || "",
        test_name: test.test_name || "",
        results: typeof test.results === 'object' ? JSON.stringify(test.results, null, 2) : test.results || "",
        date_taken: test.date_taken || "",
        notes: test.notes || "",
        is_primary: test.is_primary || false
      });

      // If results is an object and we have a template for this test type, use structured form
      if (typeof test.results === 'object' && getFormTemplate(test.test_type)) {
        setDynamicResults(test.results);
    ...
    return currentTemplate.fields.every(field => {
      if (field.required) {
        ...
        if (field.type === 'number') {
          // Check if value is a valid number and not empty/null
          return value !== null && value !== undefined && !isNaN(parseFloat(value)) && String(value).trim() !== '';
        }
        // For text, select, textarea: check if value is not empty/null/undefined
  ...
  const handleSubmit = async (e) => {
    ...
    if (useStructuredForm && currentTemplate) {
      if (!isDynamicFormValid()) {
        const missingFields = currentTemplate.fields.filter(field => field.required && (!dynamicResults[field.name] || String(dynamicResults[field.name]).trim() === ''));
    ...
    try {
      ...
      await PersonalityTest.update(testId, testData);
      navigate(createPageUrl("PersonalityTests"));
    } catch (error) {
      console.error("Error updating personality test:", error);
      alert("There was an error updating your test results. Please try again.");
    } finally {
    ...
    <div className="min-h-screen p-6 md:p-8 bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div>
            <h1 className="text-3xl font-bold text-stone-800">Edit Personality Test Result</h1>
            <p className="text-stone-600 mt-1">Update your personality assessment results</p>
          </div>
        </motion.div>
          ...
          <Card className="bg-white/90 backdrop-blur-sm border-stone-200/60 shadow-xl">
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Submit Buttons */}
                <div className="flex flex-col sm:flex-row justify-end gap-3 pt-6 border-t border-stone-200">
                  <Button
                    type="button"
                    variant="outline"
                    onClick={() => navigate(createPageUrl("PersonalityTests"))}
